...
bool routing_manager_impl::offer_service(client_t _client, service_t _service, instance_t _instance, major_version_t _major,
...

    {
        ...
        if (is_external_routing_ready()) {
            ...
            VSOMEIP_INFO << "rmi::" << __func__ << " added service: " << std::setw(4) << std::setfill('0') << std::hex << _service
            ...
            if (its_service_endpoint) {
                ...
                if (!_reliable) {
                    ...
                    if (its_server_endpoint) {
                        ...
                        VSOMEIP_INFO << "Joining multicast group: " << _address;
...
void routing_manager_impl::memory_log_timer_cbk(boost::system::error_code const& _error) {
    ...
    if (EOF
        == std::fscanf(its_file, "%" PRIu64 "%" PRIu64 "%" PRIu64 "%" PRIu64 "%" PRIu64 "%" PRIu64 "%" PRIu64, &its_size, &its_rsssize,
                       &its_sharedpages, &its_text, &its_lib, &its_data, &its_dirtypages)) {
        VSOMEIP_ERROR << "memory_log_timer_cbk: error reading:" << std::string(std::strerror(errno));
    }
    std::fclose(its_file);
